 ...
 */
#include "RTU_Adafruit_LiquidCrystal.h"
#include "RTU_Adafruit_PCF8574.h"
...
bool Adafruit_LiquidCrystal::begin(uint8_t cols, uint8_t lines,
  ...
  // check if i2c
  if (_i2cAddr != 255) {
    if (!pcf.begin(_i2cAddr, _wire))
      return false;

    for (uint8_t i = 0; i < 4; i++)
      _pinMode(_data_pins[i], OUTPUT);

    pcf.pinMode(_rs_pin, OUTPUT);
    pcf.pinMode(_enable_pin, OUTPUT);
...
/* PCF PIN USAGE */
void Adafruit_LiquidCrystal::pcf3High() {
    _digitalWrite(3, HIGH);
}
void Adafruit_LiquidCrystal::pcf3Low() {
    _digitalWrite(3, LOW);
}
